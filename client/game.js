...
function initServerScreenButtons() {
    ...
    // Toggle player list (note: players won't show until in game)
...
function exitServer() {
    // Confirm exit
    if (!confirm('Вы уверены что хотите выйти из сервера?')) {
        return;
    }
    ...
    if (playerListPanel) playerListPanel.classList.remove('open');

    const togglePlayerList = document.getElementById('toggle-player-list');
    if (togglePlayerList) togglePlayerList.classList.remove('active');

    // Return to server selection
    console.log('Redirecting to server selection...');
    showScreen('server-selection');
    renderServerList();
}
...
class Player {
    ...
    unlockAbilityForLevel(level) {
        // Define abilities for each level (1-30)
        const levelAbilities = {
            ...
            11: { key: '1', name: 'Комета', damage: 80, cooldown: 6000, color: '#e74c3c', icon: '☄️', type: 'attack' },
            12: { key: '2', name: 'Барьер', damage: 0, cooldown: 12000, color: '#ecf0f1', icon: '⭕', type: 'shield' },
            13: { key: '3', name: 'Вихрь', damage: 100, cooldown: 8000, color: '#16a085', icon: '🌀', type: 'attack' },
            14: { key: '4', name: 'Регенерация', damage: 0, cooldown: 20000, color: '#27ae60', icon: '💚', type: 'heal' },
            15: { key: '5', name: 'Буря', damage: 110, cooldown: 9000, color: '#2980b9', icon: '⛈️', type: 'attack' },
            16: { key: '6', name: 'Землетрясение', damage: 130, cooldown: 11000, color: '#795548', icon: '🌋', type: 'attack' },
            17: { key: '7', name: 'Невидимость', damage: 0, cooldown: 15000, color: '#bdc3c7', icon: '👻', type: 'stealth' },
            18: { key: '8', name: 'Взрыв света', damage: 140, cooldown: 10000, color: '#f39c12', icon: '💫', type: 'attack' },
            19: { key: '9', name: 'Яд', damage: 70, cooldown: 7000, color: '#8e44ad', icon: '☠️', type: 'poison' },
            20: { key: '0', name: 'Заморозка', damage: 90, cooldown: 8000, color: '#3498db', icon: '🧊', type: 'freeze' },
            21: { key: 'a', name: 'Призыв', damage: 100, cooldown: 25000, color: '#9b59b6', icon: '👹', type: 'summon' },
            22: { key: 's', name: 'Торнадо', damage: 160, cooldown: 13000, color: '#34495e', icon: '🌪️', type: 'attack' },
            23: { key: 'd', name: 'Лава', damage: 170, cooldown: 14000, color: '#e67e22', icon: '🔥', type: 'attack' },
            24: { key: 'f', name: 'Шторм', damage: 180, cooldown: 15000, color: '#1abc9c', icon: '⚡', type: 'attack' },
            25: { key: 'g', name: 'Аура смерти', damage: 150, cooldown: 20000, color: '#2c3e50', icon: '💀', type: 'aura' },
            26: { key: 'h', name: 'Мега щит', damage: 0, cooldown: 30000, color: '#ecf0f1', icon: '🛡️', type: 'shield' },
            27: { key: 'j', name: 'Кровожадность', damage: 0, cooldown: 25000, color: '#c0392b', icon: '🩸', type: 'buff' },
            28: { key: 'k', name: 'Черная дыра', damage: 220, cooldown: 20000, color: '#000000', icon: '🕳️', type: 'ultimate' },
            29: { key: 'l', name: 'Время', damage: 0, cooldown: 40000, color: '#95a5a6', icon: '⏰', type: 'time' },
            30: { key: 'z', name: 'Апокалипсис', damage: 300, cooldown: 60000, color: '#e74c3c', icon: '💥', type: 'ultimate' }
        };

        const newAbility = levelAbilities[level];
        if (newAbility) {
            const ABILITY_COST = 4444;

            // Check if player has enough evoluta
            if (this.evoluta >= ABILITY_COST) {
                // Deduct cost
                this.evoluta -= ABILITY_COST;
                ...
                // Show notification
                showAbilityUnlockedNotification(level, newAbility);

                // Update HUD to show new evoluta count
                updateHUD();
            } else {
                // Not enough evoluta
                showInsufficientEvolutaNotification(newAbility, ABILITY_COST);
            }
        }
    }
...
function showAbilityUnlockedNotification(level, ability) {
    ...
    notification.innerHTML = `
        ...
        <div style="font-size: 20px; color: #ffd700;">Способность разблокирована!</div>
        ...
        <div style="font-size: 14px; color: #ff6b6b; margin-top: 10px;">-4444 💎 Эволюта</div>
    `;
...
function showInsufficientEvolutaNotification(ability, cost) {
    // Create notification element
    const notification = document.createElement('div');
    notification.style.cssText = `
        position: fixed;
    ...
    notification.innerHTML = `
        <div style="font-size: 48px; margin-bottom: 10px;">❌</div>
        <div style="font-size: 20px;">Недостаточно эволюты!</div>
        <div style="font-size: 36px; margin: 15px 0;">${ability.icon}</div>
        <div style="font-size: 18px; margin-top: 10px;">${ability.name}</div>
        <div style="font-size: 16px; color: #ffd700; margin-top: 10px;">Нужно: ${cost} 💎</div>
        <div style="font-size: 14px; color: #ddd; margin-top: 5px;">У вас: ${GAME.player.evoluta} 💎</div>
        <div style="font-size: 12px; color: #ecf0f1; margin-top: 15px;">Убивайте врагов для получения эволюты!</div>
    `;
...
function initSocket() {
    // Clear any simple player objects from server selection screen
    GAME.otherPlayers.clear();
...
function initPlayerListAndFriends() {
    console.log('✅ initPlayerListAndFriends called');

    // Load friends from localStorage
    loadFriends();

    // Toggle player list
    const toggleBtn = document.getElementById('toggle-player-list');
    if (toggleBtn) {
    ...
    if (closeBtn) {
        closeBtn.addEventListener('click', () => {
            console.log('Close player list clicked');
    ...
    // Exit server button
    const exitBtn = document.getElementById('exit-server-btn');
    if (exitBtn) {
